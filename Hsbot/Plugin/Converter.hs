...
showUnits :: IRC.Message -> [String] -> Plugin (Env IO) ()
showUnits msg _ = answerMsg msg "Unit conversiion definitions: https://gist.github.com/Sornaensis/e9139ac0859967c6ead4"

doExpr :: IRC.Message -> [String] -> Plugin (Env IO) ()
doExpr _   []            = return ()
doExpr msg ("def":expr)  =
    ...
    doExpr' defs expr = 
        ...
        let conv = if isRight (readDefinitions $ egoBotDefs ++ "\n" ++ unwords defs) 
                     then egoBotFromString' ("\n" ++ unwords defs) $ unwords expr 
                     else egoBotFromString $ unwords expr
...
doExpr msg ("find":expr) = answerMsg msg $ getExpr $ unwords expr
                           where
...
doExpr msg expr          =
    ...
    where
    conv = egoBotFromString $ unwords expr
