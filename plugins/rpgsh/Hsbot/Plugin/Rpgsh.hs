...
theRpg :: Plugin (Env IO) ()
theRpg = forever $ readMsg >>= eval 
  where
...
doEval :: IRC.Message -> [String] -> Plugin (Env IO) (Maybe PluginRet)
doEval msg xs = case readExpr (unwords xs) of
                    Just str -> answerMsg msg str >> return Nothing
                    _        -> return Nothing
...
evalExpr :: ExprVal -> Maybe String
evalExpr (String str) = Just str
evalExpr (Param name) = Just $ "%" ++ name
evalExpr (EList [])   = Nothing
evalExpr (EList xs)   = last $ map evalExpr xs
evalExpr (List xs)    = last $ map evalExpr xs
evalExpr x            = Just $ show x
...
readExpr input = case parse parseExprs "hsbot2::eval" input of
                    ...
                    Right val -> last (map evalExpr val) 
