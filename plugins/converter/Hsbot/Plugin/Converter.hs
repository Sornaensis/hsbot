...
-- | The unitConvert plugin identity
converter :: PluginId
converter = PluginId
    { pluginName = "converter"
    , pluginEp   = theUnitConvert 
...
theCmds = [
            ...
            , cmdFunc = doExpr
            , cmdPerm = Nothing
            , cmdHelp = "Convert units. SYNTAX: convert [def [UNIT DEFINITION] :] [EXPRESSION] => [TARGET UNIT] / See \'converter units\' for units descriptions"
            }
          , PluginCmd
            { command = "units"
            , cmdIsPrefix = False
            , cmdFunc = showUnits
            , cmdPerm = Nothing
            , cmdHelp = "Unit conversion definitions: https://gist.github.com/Sornaensis/e9139ac0859967c6ead4" }
          
          ]

showUnits :: IRC.Message -> [String] -> Plugin (Env IO) (Maybe PluginRet)
showUnits msg _ = answerMsg msg "Unit conversion definitions: https://gist.github.com/Sornaensis/e9139ac0859967c6ead4" >> return Nothing

doExpr :: IRC.Message -> [String] -> Plugin (Env IO) (Maybe PluginRet)
doExpr _   []            = return Nothing
doExpr msg ("def":expr)  =
    doDefExpr (L.elemIndex ':' $ unwords expr) (expr)
    >> return Nothing
    where
    doExpr' defs expr = 
        do
        let conv = if isRight (readDefinitions $ egoBotDefs ++ "\n" ++ unwords defs) 
                     then egoBotFromString' ("\n" ++ (map toLower $ unwords defs)) $ map toLower $ unwords expr 
                     else egoBotFromString $ map toLower $ unwords expr
        answerMsg msg $ if isRight conv 
                          then show $ head $ rights [conv] 
                          else "Error with unit conversion \x1F" ++ show (head $ lefts [conv]) ++ "\x1F"

    doDefExpr Nothing _ = answerMsg msg "Syntax: .convert def <definitions> : <expression>"
    doDefExpr (Just n) xs = do
                        let (defs, expr) = splitAt n $ unwords xs
                        doExpr' (words defs) (words $ tail expr)
doExpr msg ("find":expr) = (answerMsg msg $ getExpr $ map toLower $ unwords expr)
                           >> return Nothing
                           where
                           getExpr a = loop $ lines egoBotDefs
                                       where 
                                       loop []     = "No matching unit"
                                       loop (x:xs) = if a `L.isInfixOf` x then x else loop xs 
doExpr msg expr          =
    do
    answerMsg msg $ (if isRight conv 
                     then show . head $ rights [conv] 
    ...
    return Nothing
    where
    conv = egoBotFromString $ map toLower $ unwords expr

-- | An IRC plugin for converting lots of units
theUnitConvert :: Plugin (Env IO) ()
theUnitConvert = forever $ readMsg >>= eval
  where
...
egoBotDefs = unlines [
    ...
    -- reference
    ,"meter = [length] = m = meters = metre"
    ,"second = [time] = s = seconds = sec"
    ...
    -- Time
    ,"minute = 60 * second = min = minutes = mins"
    ,"hour = 60 * minute = h = hr = hrs = hours"
    ,"day = 24 * hour = days"
    ,"week = 7 * day = weeks"
    ,"fortnight = 2 * week = fortnights"
    ,"year = 31556925.9747 * second = years"
    ,"month = year/12 = months"
    ,"shake = 1e-8 * second = shakes"
    ...
    ,"mile = 5280 * foot = miles = mi = international_mile"
    ,"yard = 3 * feet = yd = yards = international_yard"
    ,"mil = inch / 1000 = thou"
    ,"parsec = 3.08568025e16 * meter = pc"
    ,"light_year = speed_of_light * julian_year = ly = lightyear = lightyears"
    ...
    -- Mass
    ,"ounce = 28.349523125 * gram = oz = avoirdupois_ounce = ounces"
    ,"dram = oz / 16 = dr = avoirdupois_dram"
    ,"pound = 0.45359237 * kilogram = lb = avoirdupois_pound = lbs = pounds"
    ,"stone = 14 * lb = st"
    ,"carat = 200 * milligram"
    ...
    ,"newton = kilogram * meter / second ** 2 = N = newtons"
    ...
    -- Power
    ,"watt = joule / second = W = volt_ampere = VA = w"
    ...
    -- More Energy
    ,"watt_hour = watt * hour = Wh = watthour = wh"
    ...
    -- Velocity
    -- [speed] = [length] / [time]
    ,"knot = nautical_mile / hour = kt = kts = knots = knot_international = international_knot = nautical_miles_per_hour"
    ,"mph = mile / hour = MPH"
    ...
    -- Volume
    -- [volume] = [length] ** 3
    ,"liter = 1e-3 * m ** 3 = l = L = litre = liters = litres"
    ...
    ,"gallon = 231 * inch ** 3 = liquid_gallon = US_liquid_gallon = gallons"
    ,"quart = gallon / 4 = liquid_quart = US_liquid_quart = quarts"
    ,"pint = quart / 2 = pt = liquid_pint = US_liquid_pint = pints"
    ,"cup = pint / 2 = liquid_cup = US_liquid_cup = cups"
