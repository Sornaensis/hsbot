...
parseCommand :: IRC.Message -> [String] -> PluginCmd -> Plugin (Env IO) (Maybe PluginRet)
parseCommand _ [] _ = return Nothing
parseCommand msg (arg:args) (PluginCmd cmd isPrefix func permission _) =
    ...
    case access of
        True -> case isPrefix of
                    True -> if cmd `L.isPrefixOf` arg then func msg (words $ unwords $ (L.drop (L.length cmd) arg) : args) else return Nothing
                    _    -> if cmd == arg then func msg args else return Nothing
        _    -> return Nothing
...
hasAccess (Just mask) (Just right) =
  ...
  where
    ...
    accessMatch (AccessList amask arights)
      | mask == amask = (right <= minimum arights)
      | otherwise = False
